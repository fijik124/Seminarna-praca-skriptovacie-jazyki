...
function tracks_page_data(?PDO $pdo = null): array {
	$availableTags = track_default_tags();
	$regions = track_regions();
	$tracks = track_resolve_listing_tracks($pdo);
...
function tracks_query_filters(array $availableTags, array $regions): array {
	...
	if (!in_array($day, track_days(), true)) {
		$day = '';
	}
...
function tracks_open_days_count(array $track): int {
	$count = 0;
...
function tracks_earliest_opening(array $track): int {
	$min = 9999;
	foreach (track_days() as $day) {
...
function tracks_latest_closing(array $track): int {
	$max = -1;
	foreach (track_days() as $day) {
		$slot = $track['schedule'][$day] ?? null;
		if (!$slot) {
...
function tracks_is_open_at(array $track, string $day, string $hour): bool {
	if ($day === '' || $hour === '') {
		return true;
	}
...
function tracks_filter_tracks(array $tracks, array $filters): array {
	...
	return array_values(array_filter($tracks, static function (array $track) use ($filters, $search): bool {
...
function tracks_sort_tracks(array &$tracks, string $sortBy): void {
	usort($tracks, static function (array $a, array $b) use ($sortBy): int {
		if ($sortBy === 'openDays') {
...
$pageData = tracks_page_data(isset($pdo) && ($pdo instanceof PDO) ? $pdo : null);
		...
		<form method="get" action="<?= htmlspecialchars($formAction) ?>" class="tracks-panel p-4 mb-4">
			<div class="row g-3 align-items-end">
				<div class="col-md-6 col-xl-3">
					<label class="form-label fw-semibold">Search Track</label>
					<input type="text" name="search" class="form-control filter-input" placeholder="Name or city" value="<?= htmlspecialchars($filters['search']) ?>">
				</div>
				<div class="col-md-6 col-xl-3">
					<label class="form-label fw-semibold">Region (Kraj)</label>
				...
				<div class="col-md-6 col-xl-2">
					<label class="form-label fw-semibold">Open Day</label>
					<select name="day" class="form-select filter-input">
						<option value="">Any day</option>
						<?php tracks_render_day_options($filters['day']); ?>
					</select>
				</div>
				<div class="col-md-6 col-xl-2">
					<label class="form-label fw-semibold">At Hour</label>
					<input type="time" name="hour" class="form-control filter-input" value="<?= htmlspecialchars($filters['hour']) ?>">
				</div>
				<div class="col-xl-2">
			...
			<div class="mt-4 d-flex flex-wrap gap-2">
				...
				<a href="/tracks" class="btn btn-outline-light">Reset</a>
			</div>
		</form>
